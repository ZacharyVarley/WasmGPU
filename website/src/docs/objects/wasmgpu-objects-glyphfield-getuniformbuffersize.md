# GlyphField.getUniformBufferSize

## Summary
GlyphField.getUniformBufferSize returns the current uniform buffer size value derived from this GlyphField runtime state.

## Syntax
```ts
GlyphField.getUniformBufferSize(): number
const result = glyphField.getUniformBufferSize();
```

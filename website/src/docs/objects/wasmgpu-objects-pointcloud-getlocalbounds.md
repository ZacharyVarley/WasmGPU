...
## Summary
PointCloud.getLocalBounds returns the current local bounds value derived from this PointCloud runtime state.

## Syntax
```ts
PointCloud.getLocalBounds(): Bounds3
const result = pointCloud.getLocalBounds();
```
